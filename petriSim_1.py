...
CIRCLE_RADIUS = min(SCREEN_WIDTH, SCREEN_HEIGHT) // 2 - 25  # Example radius
...
# Counters for spawned/generated bacteria
# Super-Orange and Blue bacteria will be counted once in the first initiation.
total_red_bacteria_spawned = 1 # Super-Orange bacteria will be counted once.
total_green_bacteria_spawned = 1
...
def draw_bacterial_counts():
    red_bacteria_text = game_font.render(f"Red Bacteria: {len(red_bacteria)+1}", True, text_color)
    green_bacteria_text = game_font.render(f"Green Bacteria: {len(green_bacteria)+1}", True, text_color)
    screen.blit(red_bacteria_text, (5, 5))
    screen.blit(green_bacteria_text, (5, 5 + font_size + 5))
...
def draw_banner():
    if game_state == "start":
        ...
        message = f'You won! Total Red Spawned: {total_red_bacteria_spawned}, Total Green Spawned: {total_green_bacteria_spawned}. Press "r" to restart the game.'
    elif game_state == "lost":
        message = f'You lost! Total Red Spawned: {total_red_bacteria_spawned}, Total Green Spawned: {total_green_bacteria_spawned}. Press "r" to restart the game.'
    else:
...
total_red_bacteria_spawned += 5  # Initial spawn
...
def check_collisions_and_multiply():
    ...
    global total_green_bacteria_spawned, total_red_bacteria_spawned
    ...
    for food in food_list[:]:
        if player_bacteria and player_bacteria.colliderect(food):
            ...
            for _ in range(MULTIPLY_RATE - 1):
                new_bacteria = pygame.Rect(player_bacteria.x + random.randint(-10, 10), 
                                           player_bacteria.y + random.randint(-10, 10), 
                                           BACTERIA_SIZE, BACTERIA_SIZE)
                green_bacteria.append(new_bacteria)
                total_green_bacteria_spawned += 1  # Increment here inside the loop
    ...
    for green in green_bacteria[:]:
        for food in food_list[:]:
            if green.colliderect(food):
                ...
                for _ in range(MULTIPLY_RATE - 1):  # Assume MULTIPLY_RATE is how many total you want including the original
                    new_bacteria = pygame.Rect(green.x + random.randint(-10, 10),
                                               green.y + random.randint(-10, 10),
                                               BACTERIA_SIZE, BACTERIA_SIZE)
                    green_bacteria.append(new_bacteria)
                    total_green_bacteria_spawned += 1  # Each multiplication
    ...
    for red in red_bacteria[:]:
        for food in food_list[:]:
            if red.colliderect(food):
                ...
                total_red_bacteria_spawned += 1  # Each multiplication
